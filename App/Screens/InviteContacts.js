...
const textMessage = 'Start chatting with me on Tilt. Join now at https://4yfnj.app.link/f1zU8qPIyZ';
...
const openSmsUrl = (phone = '') => () => Linking.openURL(`sms:${phone}${getSMSDivider()}body=${textMessage}`);

const openEmail = () => Linking.openURL(`mailto:?subject=${emailSubject}&body=${textMessage}`);
...
const getContactsAndroid = () => new Promise((resolve, reject) => {
  PermissionsAndroid.request(
    PermissionsAndroid.PERMISSIONS.READ_CONTACTS,
    {
      title: 'Contacts',
      message: 'Tilt wants access to display your contacts. This app cannot send messages without your explicit permission.'
    }
  )
    .then(() => {
...
const getContactsIos = () => new Promise((resolve, reject) => {
...
class InviteContacts extends React.PureComponent {
    static navigationOptions = ({ navigation }) => ({
      ...
      headerLeft: <GoBack icon={BACK} onPress={() => navigation.dispatch(NavigationActions.back())} />
    })
    ...
    render() {
      ...
      const data = contacts.filter(({ givenName, phoneNumbers, familyName }) => {
        const hasGivenName = givenName ? givenName.toLowerCase().includes(search) : false;
        const hasPhoneNumber = phoneNumbers[0] ? phoneNumbers[0].number.toLowerCase().includes(search) : false;
        const hasFamilyName = familyName ? familyName.toLowerCase().includes(search) : false;
