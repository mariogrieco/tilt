...
import {
  getAllRootsByChannelId
} from '../selectors/getAllRootsforPost';
...
class Thread extends React.Component {
  static navigationOptions = ({ navigation }) => ({
    title: 'Edit Message',
    headerLeft: (
      <GoBack icon={BACK} onPress={() => navigation.dispatch(NavigationActions.back())} />
    )
  })
    ...
    render() {
      ...
      const keyboardVerticalOffset = Platform.OS === 'ios' ? ifIphoneX(88, 60) : 0;

      return (
        <SafeAreaView style={{ flex: 1 }}>
          <ScrollView keyboardDismissMode="on-drag" contentContainerStyle={{ flex: 1, paddingTop: 11 }}>
