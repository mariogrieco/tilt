...
const renderItem = ({ item }) => (
  <View>
    <View
      style={[styles.listContainer, styles.container]}
    >
      <Text style={[styles.listItem, { flex: 0.8 }]}>{parseFloat(item.bid.qty)}</Text>
      <View style={{ flex: 0.2, alignItems: 'flex-start', justifyContent: 'center' }}>
        <Image source={CROSS} />
      </View>
      <Text style={[styles.listItem, { flex: 1, textAlign: 'center', color: '#17C491' }]}>{`${parseFloat(item.bid.price)}`}</Text>
      <Text style={[styles.listItem, { flex: 1, textAlign: 'center', color: '#FC3E30' }]}>{`${parseFloat(item.ask.price)}`}</Text>
      <View style={{ flex: 0.2, alignItems: 'flex-end', justifyContent: 'center' }}>
        <Image source={CROSS} />
      </View>
      <Text style={[styles.listItem, { flex: 0.8, textAlign: 'right' }]}>{parseFloat(item.ask.qty)}</Text>
    </View>
  </View>
  ...
  render() {
    ...
    return (
      <View>
        ...
        <View
          style={[styles.headerContainer, styles.container]}
        >
