...
const PostMediaModal = ({ postMedia, closedModal, hideModal }) => {
  ...
  const download = async () => {
    try {
      const { link } = await Client4.getFilePublicLink(postMedia.id).catch(err => console.log(err));
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={{ flex: 1, position: 'relative' }}>
          ...
          >
            ...
            <TouchableOpacity style={{ paddingHorizontal: 15, paddingBottom: 15 }} onPress={download}>
...
export default connect(mapStateToProps, mapDispatchToProps)(PostMediaModal);
