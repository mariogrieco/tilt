...
class PublicSearch extends React.Component {
    ...
    handleTogglePress = currentToggleSelected => this.setState({ currentToggleSelected })
      ...
      return (
        <ScrollView>
          ...
          </View>
          {
                currentToggleSelected === 'left'
                  ? (channels
                    .filter(channel => this.getParseName(channel).includes(searchValue))
                    .map(channel => (
                      <ChannelDisplayName
                        show
                        channel={channel}
...
const mapStateToProps = (state) => {
  ...
  getChannelsList(state).forEach((channel) => {
    ...
    mapFavoritesChannels[channel.id] = getFavoriteChannelById(state, channel.id);
    mapChannelsColors[channel.id] = channel.titleColor;
  });
  const sponsorIds = state.sponsored;
  const usersWithOutSponsored = Object
    .values(users.data)
    .filter(user => !sponsorIds.includes(user.id));
...
export default connect(mapStateToProps, mapDispatchToProps)(PublicSearch);
