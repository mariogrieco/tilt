...
const posts = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_POST_TO_SUCCESS: {
      if (!state.orders[action.payload.channel_id] && !state.entities[action.payload.id]) return state;
      const nextState = cloneDeep(state);
      nextState.orders[action.payload.channel_id].page_order[0].unshift(action.payload.id);
      nextState.orders[action.payload.channel_id].order.unshift(action.payload.id);
    ...
    case REMOVE_FROM_POST_SUCCESS: {
      ...
      if (nextState.orders[channel_id]) {
        nextState.orders[channel_id].order = nextState.orders[channel_id].order.filter(key => key !== postId);
        nextState.orders[channel_id].page_order = nextState.orders[channel_id].page_order.map(orders => orders.filter(i => i !== postId));
      }
