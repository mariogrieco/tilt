...
const getThreadForPost = (state, postData) => {
  ...
  let active = false;
  const data = [...(channelData && channelData.order ? channelData.order : [])].reverse();
  ...
  return postThreads.map((post, index) => {
    let replies = 0
    if (index === 0) {
      replies = getAllRootsforPost(channelData && channelData.order ? channelData.order : [], entities, post.id);
    }
    return {
      ...post,
      replies,
